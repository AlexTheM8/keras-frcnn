...
parser.add_option("--rot", "--rot_90", dest="rot_90",
                  ...
                  action="store_true", default=False)
parser.add_option("--num_epochs", type="int", dest="num_epochs", help="Number of epochs.", default=15)
...
parser.add_option("--input_weight_path", dest="input_weight_path",
                  help="Input path for weights.", default='./model_frcnn.hdf5')
...
for epoch_num in range(num_epochs):
    ...
    while iter_num != epoch_length:
        try:
            ...
            if str(e) != "'a' cannot be empty unless no samples are taken":
                print(f'Exception: {e}')
            continue
    ...
    acc_list.append(int((1 - curr_loss) * 100))
...
for f in os.listdir('./'):
    if f.endswith(model_path_regex.group(2)) and len(f.split('_')) == 3:
        fullname = f.split('.')
        accuracy = acc_list[int(fullname[0].split("_")[2])]
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        new_name = '_'.join([fullname[0], str(accuracy), timestamp])
        if not os.path.exists(f'./history/{time.strftime("%Y%m%d")}/'):
            os.mkdir(f'./history/{time.strftime("%Y%m%d")}/')
        shutil.copy(f, f'./history/{time.strftime("%Y%m%d")}/{new_name}.{fullname[1]}')
        if int(fullname[0].split("_")[2]) + 1 == num_epochs:
            try:
                os.remove(f"{'_'.join(fullname[0].split('_')[:-1])}.{fullname[1]}")
            except OSError:
                pass
            os.rename(f, f"{'_'.join(fullname[0].split('_')[:-1])}.{fullname[1]}")
        else:
            os.remove(f)
