...
parser.add_option("--network", dest="network", help="Base network to use. Supports vgg or resnet50.",
                  default='resnet50')
parser.add_option("-v", "--visualize", dest="visualize", help="Whether to save results with bounding boxes",
                  default='True', choices=('True', 'False'))
parser.add_option("-o", "--overlap_threshold", type="float", dest="overlap_threshold",
                  help="Percent of allowed overlap with bounding boxes", default=0.5)
...
if num_features == 0:
    raise ValueError(f'num_features remained 0 due to network value: {C.network}')
...
visualise = str.lower(options.visualize) == 'true'
...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
    ...
    for key in bboxes:
        ...
        # Note: this line is actual result, everything else is visualization & can be removed
        ...
        for jk in range(new_boxes.shape[0]):
            ...
            if visualise:
                ...
                print(f'{img_name} goal found at ({real_x1}, {real_y1}, {real_x2}, {real_y2}) '
                      f'with confidence {100 * new_probs[jk]}')
